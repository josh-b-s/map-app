...
import { setQuery, searchPlaces, selectPlace, clearResults } from '@/store/search.slice';
...
export default function Search() {
    ...
    const onSelect = useCallback(async (place: any) => {
        ...
        Keyboard.dismiss();
        if (!userLocation) { Alert.alert('Waiting for user location'); return; }
        dispatch(computeRoute({ origin: userLocation as LatLng, destination: { latitude: place.latitude, longitude: place.longitude }, apiKey: GOOGLE_MAPS_APIKEY, travelMode: 'DRIVE' }));
    ...
    return (
        <View style={styles.searchContainer} pointerEvents="box-none">
            <View style={styles.searchBar}>
                ...
                <TextInput value={query} onChangeText={onChange} placeholder="Search places..." style={styles.searchInput} returnKeyType="search" />
                {(query.length > 0 || loading) && (
                    <TouchableOpacity onPress={() => { dispatch(setQuery('')); dispatch(clearResults()); Keyboard.dismiss(); }} style={styles.clearButton}>
